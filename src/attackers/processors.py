...
class SpoofedProcessor(LogitsProcessor):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.counts_base = counts_base#基础文本统计
        self.counts_wm = counts_wm#水印文本统计
        self.counts_wm_ft = counts_wm_ft#微调的带水印文本统计
        self.prevctx_width = prevctx_width#上下文窗口大小
    ...
    def __call__(  # noqa: C901
        ...
        if input_ids.shape[1] < self.prevctx_width:
            return logits
        '''
            print('开始伪造水印 这里是processors.py->__call__方法-----处理器')
            print(input_ids)
            print(logits)
            print(f'第{SpoofedProcessor.INDEX}次调用')
            SpoofedProcessor.INDEX+=1
            print('-----结束')


            这里的调用链attack.generate(process,prompt)->hf(process=spoofing)回调
        '''
        ...
        for b in range(input_ids.shape[0]):
            ...
            # 1) {ABC}->D: most precise but generally sparse   使用完整的上下文
            # print('这里获取完整的上下文')
            ...
            if self.cfg.w_partials > 0:#1.0    使用部分上下文
                # print('这里获取部分上下文')
                ...
                # To be strong you need to have higher cossim with pair boost than all  找到最强的,最有可能表示水印的token
                winner = -1 
                ...
                # If there was a unique winner add its boost  如果找到了,那就把它加到boosts中
                if winner > -1:
                    boosts += self.cfg.w_partials * solo_boosts[winner]
                    total_w += self.cfg.w_partials

            if self.cfg.w_empty > 0:#0.5  使用空上下文
                # print('这里获取空上下文')
            ...
            if self.cfg.w_future > 0:#0,不执行 (这里涉及重参数采样,并且需要roberta模型,暂时不执行)
            ...
            # Average to get the final boosts   归一化,这里就是论文中提到的若干情况的加权求和
            boosts /= total_w

...
class OurAttacker(BaseAttacker):
    ...
    def load_queries_and_learn(self, base: bool = False) -> None:
        ...
        # Find the counts cache closest to nb_queries 这里是在寻找pkl文件
    ...
    def get_processor_list(
        ...
        print(cfg_gen)
        print('----------')
        # Build processor list创建伪造水印的处理器
        processors = []
        #重复惩罚
        ...
        #伪造水印
        print('开始伪造水印 -----配置信息')
        print(self.counts_base)
        print(self.counts_wm)
        print(self.counts_wm_ft)
        print(self.prevctx_width)
        ...
        if cfg_gen.spoofer_strength != 0:
            print('这里是ourattacker.py三个步骤中的第二个,伪造水印')
            processors.append(
                SpoofedProcessor(
    ...
    def generate(
        ...
        #获取处理器列表,包含了水印伪造的逻辑
        #利用这些处理器调用模型进行生成  
        # 水印伪造的关键在于调用了get_processor_list的方法,返回一系列修改概率生成的处理器
        print('这里是get_processor_list方法的入口,,,获取processor list')
        ...
        if "dipper" in self.model.cfg.name:
            ...
            print('这里是our_attacker.py的generate方法')

...
class Evaluator:
    ...
    def eval_targeted_spoofing(self, attacker: BaseAttacker, eval_mode: EvalMode) -> List[str]:
        ...
        print(f'gcg advbench 数据集读取完毕,共{len(prompts)}个prompt')
        ...
        for b in range(nb_batches):#b表示第b个batch,nb_batches表示batch的总数
            ...
            #这里返回的就是模型的输出,带水印的东西,打印出来是粉色的
            completions = attacker.generate(prompts=inputs, reseed=reseed)

            print('attacker.generate完成')
            print('输出completions-----------------------',completions)
    ...
    def _run_eval(
        ...
        if eval_mode == EvalMode.PROXY:
            ...
            if eval_class == EvalClass.SCRUBBING or eval_class == EvalClass.RUBBINGIN:
                ...
                print('开始评估targeted_spoofing')
                results = self.eval_targeted_spoofing(attacker, eval_mode)######当前调用的就是这个了
        return results

    def run_eval(self, server: Server, attacker: BaseAttacker, out_dir: str) -> None:
        #Eval class to run -- spoofing or scrubbing Eval_class的作用
        eval_class, eval_mode = self.cfg.eval_class, self.cfg.eval_mode
        print('run_eval开始评估')
        print(eval_class,eval_mode)
        results = self._run_eval(eval_class, eval_mode, server, attacker)
        with create_open(
            os.path.join(
                out_dir,#这里的地址是txt的上一级
